"""src/features/signal_processor.py -- convert RedrobSignals into a normalized feature dict.

ALL thresholds and normalization values come from config; no literal
judgment numbers appear in this module.
"""
...
def _open_to_work_score(candidate: Candidate) -> float:
    return (
...
def _notice_period_score(candidate: Candidate) -> float:
...
def _recent_activity_score(candidate: Candidate, today: date) -> float:
...
def _response_speed_score(candidate: Candidate) -> float:
...
def _profile_views_score(candidate: Candidate) -> float:
...
def _saved_by_recruiters_score(candidate: Candidate) -> float:
...
def _skill_assessments_score(candidate: Candidate) -> float:
...
def _github_activity_score(candidate: Candidate) -> float:
...
def _location_fit_score(candidate: Candidate) -> float:
...
def _willing_to_relocate_score(candidate: Candidate) -> float:
    return (
...
def _experience_fit_score(candidate: Candidate) -> float:
...
def process(candidate: Candidate, today: date) -> Dict[str, float]:
    ...
    return {
        "open_to_work": _open_to_work_score(candidate),
        "notice_period_score": _notice_period_score(candidate),
        "recent_activity": _recent_activity_score(candidate, today),
        ...
        "profile_views": _profile_views_score(candidate),
        "saved_by_recruiters": _saved_by_recruiters_score(candidate),
        "profile_completeness": candidate.redrob_signals.profile_completeness_score / 100.0,
        "skill_assessments": _skill_assessments_score(candidate),
        "github_activity": _github_activity_score(candidate),
        "location_fit": _location_fit_score(candidate),
        "willing_to_relocate": _willing_to_relocate_score(candidate),
