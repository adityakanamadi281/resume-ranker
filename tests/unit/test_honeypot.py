...
from resume_ranker.domain.honeypot_rules import detect
...
def test_duration_mismatch_detected(candidate_factory, today):
    ...
    is_honeypot, reasons = detect(candidate, today, config)
    assert is_honeypot
...
def test_valid_candidate_is_not_honeypot(sample_candidate, today):
    is_honeypot, reasons = detect(sample_candidate, today, config)
...
def test_education_hierarchy_inversion_detected(candidate_factory, today):
    candidate = candidate_factory(
        education=[
            {
    ...
    is_honeypot, reasons = detect(candidate, today, config)
    assert is_honeypot
    assert any("education timeline inversion" in r for r in reasons)
